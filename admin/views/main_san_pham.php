		...
		<div class="container">
			<div class="row">
				<ul class="ul-search menu-action">
					...
					<li>
						<a href="?page=product&select=delete&id=<?php echo $result['ma_sp']; ?>"><button><i class="fas fa-trash-alt"></i> Xóa</button></a>
					</li>
					<li>
				...
				<div class="pagination">
					<div class="page">
						...
						if ($current_page > 3) {
						    $first_page = 1;
						    ?>
						    <a class="page-item" href="?page=product&pagination=<?= $first_page ?>"><i class="fas fa-fast-backward"></i></a>
						    <?php
						}
						if ($current_page > 1) {
						    $prev_page = $current_page - 1;
						    ?>
						    <a class="page-item" href="?page=product&pagination=<?= $prev_page ?>"><i class="fas fa-backward"></i></a>
						<?php }
						?>
						<?php for ($num = 1; $num <= $totalPages; $num++) { ?>
						    <?php if ($num != $current_page) { ?>
						        <?php if ($num > $current_page - 3 && $num < $current_page + 3) { ?>
						            <a class="page-item" href="?page=product&pagination=<?= $num ?>"><?= $num ?></a>
						        <?php } ?>
						    <?php } else { ?>
						...
						if ($current_page < $totalPages - 1) {
						    ...
						    ?>
						    <a class="page-item" href="?page=product&pagination=<?= $next_page ?>"><i class="fas fa-forward"></i></a>
						<?php
						}
						if ($current_page < $totalPages - 3) {
						    $end_page = $totalPages;
						    ?>
						    <a class="page-item" href="?page=product&pagination=<?= $end_page ?>"><i class="fas fa-fast-forward"></i></a>
						    <?php
						}
						?>
					</div>
